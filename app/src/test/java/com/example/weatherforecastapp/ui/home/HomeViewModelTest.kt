...
class HomeViewModelTest {
    ...
    @Test
    fun getWeatherDataLocally_modelRoot_nothing() = main.runBlockingTest {
        // Given : modelRoot
        val model = modelRoot
    ...
    @Test
    fun get_WeatherDataOnline_latAndLong_true() = main.runBlockingTest {
        val flow = repo.getWeatherDataOnline(1.1, 2.2)
        val data1 = Response.success(modelRoot).body()
        flow.collect {
            if (it.isSuccessful) {
                val data = it.body()
                Truth.assertThat(data).isEqualTo(data1)
                repo.saveWeatherData(data as ModelRoot)
                val localFlow = repo.getWeatherDataLocally()
                localFlow.collect { root ->
                    Assert.assertThat(root, CoreMatchers.notNullValue())
                }
            } else {
                Assert.assertThat(it.errorBody(), CoreMatchers.notNullValue())
            }
        }
    }
